...
async function seed() {
  console.log("Iniciando sembrado de propietarios...");
  for (const hosteria of hosterias) {
    ...
    try {
      ...
      console.log(`👤 Usuario de Auth creado: ${hosteria.email}`);
    } catch (error) {
      if (error.code === 'auth/email-already-in-use') {
        console.log(`⚠️ Auth ya existe: ${hosteria.email}, recuperando UID...`);
        // We can just log in to get the UID
        const { signInWithEmailAndPassword } = await import("firebase/auth");
        const userCredential = await signInWithEmailAndPassword(auth, hosteria.email, "Sigchos2026");
        user = userCredential.user;
      } else {
    ...
    if (user) {
      try {
        ...
        await setDoc(doc(db, "propietarios", user.uid), {
