...
''';
  ...
  Future<Map<String, dynamic>> enviarMensaje(String mensaje, Map<String, dynamic> contexto, {bool isAudioText = false}) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Error Groq Chat: ${response.statusCode} - ${response.body}');
    }
  }

  Future<Map<String, dynamic>> enviarAudio(String filePath, Map<String, dynamic> contexto) async {
    final apiKey = dotenv.env['GROQ_API_KEY'];
    if (apiKey == null || apiKey.isEmpty) {
      throw Exception('Falta GROQ_API_KEY en el .env');
    }
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Error Groq Whisper: ${response.statusCode} - ${response.body}');
    }
  }
}
