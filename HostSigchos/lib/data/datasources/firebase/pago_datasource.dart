...
class PagoDataSource {
  ...
  /// Actualizar el estado de un pago en Firestore
  Future<void> actualizarEstadoPago(String pagoId, String nuevoEstado) async {
    try {
      await _firestore.collection(FirestorePaths.pagos).doc(pagoId).update({
        'estado': nuevoEstado,
      });
    } catch (e) {
      throw const FirestoreFailure('Error al actualizar el estado del pago');
    }
  }
}
